...
class SurveyViewModel(private val photoUriManager: PhotoUriManager) : ViewModel() {
    ...
    fun onFreeTimeResponse(selected: Boolean, answer: Int) {
        if (selected) _freeTimeResponse.add(answer) else _freeTimeResponse.remove(answer)
        _isNextEnabled.value = getIsNextEnabled()
    }
...
class SurveyViewModelFactory(private val photoUriManager: PhotoUriManager) :
    ViewModelProvider.Factory {
    @Suppress("UNCHECKED_CAST")
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(SurveyViewModel::class.java)) {
            return SurveyViewModel(photoUriManager = photoUriManager) as T
        }
